 ...
 */
export function buildPersonaContext(ph, lifecycleState, socialState) {
    const lines = [];
 ...
 */
export function getSocialContext(socialState) {
    const now = new Date();
    const neglected = [];
    let lonelinessUrgency = null;
 ...
 */
export function calculateMonthlyExpenses(finance) {
    let total = 0;
 ...
 */
export function calculateMonthlyIncome(finance) {
    let total = 0;
 ...
 */
export function getFinancialContext(finance) {
    const now = new Date();
 ...
 */
export function calculateUrgePriority(ph, finance, socialState, lifecycleState) {
    const urgencies = [];
    // Biological needs
 ...
 */
export function buildStateOfBeing(urgencies, ph, finance, socialState) {
    const parts = [];
    // Top priority
 ...
 */
export function buildSensoryContext(ph, lang, modules, cycleState, cycleProfile, emotionState, desireState, identityLine, growthContext, dreamState, hobbySuggestion, lifecycleState, socialState, financeState, worldState, skillState, psychState, reputationState, socialEvent) {
    const parts = [];
    if (socialEvent) {
    ...
    const newsState = globalThis._newsState;
    if (newsState?.headlines && newsState.headlines.length > 0) {
        const headlines = newsState.headlines.slice(0, 3).map((h) => `- ${h.title}`).join("\n");
    ...
    const bodyLines = [];
    const needTypes = ["bladder", "bowel", "hunger", "thirst", "energy", "hygiene", "stress", "arousal", "libido"];
    for (const type of needTypes) {
        if (type === "arousal" && !modules.eros)
            continue;
        if (type === "libido" && !modules.eros)
            continue;
        const val = ph.needs[type] ?? 0;
        const sensation = getSensationValue(val, type);
        if (sensation)
            bodyLines.push(`- ${sensation}`);
    }
    ...
    if (modules.dreams && dreamState?.active) {
        ...
        const durationStr = durationMin >= 60
            ...
            : `${durationMin}min`;
        parts.push(`[DREAM MODE ACTIVE — since ${durationStr}]\n` +
...
function getSensationValue(value, type) {
 ...
 */
export async function queryMem0(query, config, limit = 5) {
    if (!config.enabled) {
        return "";
    }
 ...
 */
export async function storeMem0Fact(fact, config) {
    if (!config.enabled) {
        return false;
    }
    const bridgeScript = join(process.cwd(), "skills", "soul-evolution", "tools", "memory_bridge.py");
 ...
 */
export function getVaultMorningReportDirective() {
    const now = new Date();
