...
export function detectAgentRole(agentId, roleMapping) {
    if (!roleMapping)
        return "persona";
...
export async function cleanupExpiredMemos(commPath, ttlDays) {
    const comm = await readJson(commPath);
    if (!comm?.memos)
        return;
    const now = new Date().getTime();
    const filtered = comm.memos.filter((m) => {
...
export async function logVitalityTelemetry(telDir, lifecycle, needs, location) {
...
export async function logAgentActivity(telDir, agentId, role, action, message, meta = {}) {
    const date = new Date().toISOString().slice(0, 10);
