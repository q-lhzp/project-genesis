 ...
 */
async function loadEconomyState(workspacePath) {
 ...
 * Save economy state
 */
async function saveEconomyState(workspacePath, state) {
    const statePath = join(workspacePath, "memory", "reality", "economy_state.json");
 ...
 */
function determineRiskTolerance(physique) {
 ...
 */
function analyzeMarketMood(physique, isResearching) {
    const stress = physique.needs.stress ?? 50;
    const joy = physique.needs.joy ?? 50;
    ...
    if (stress > 80)
        return "bearish"; // Panic selling
    if (joy > 70)
        return "bullish"; // Euphoric buying
    return "neutral";
}
/**
 * Select trading strategy based on market mood and risk tolerance
 */
function selectStrategy(mood, riskTolerance) {
 ...
 */
async function executeTrade(workspacePath, symbol, amount, tradeType, strategy, reason, mood) {
 ...
 */
async function getPortfolioStatus(workspacePath) {
    const vaultBridge = join(workspacePath, "skills", "soul-evolution", "tools", "vault_bridge.py");
    try {
        const { stdout } = await execFilePromise("python3", [
            vaultBridge,
 ...
 */
export async function processEconomy(workspacePath, physique, isResearching, config = DEFAULT_CONFIG) {
    ...
    if (state.isActive) {
        ...
        if (strategy === "momentum_buy" || strategy === "gradual_buy" || strategy === "buy_the_dip") {
            ...
            if (amount > 0.01 && portfolio.cash > 10) {
                const result = await executeTrade(workspacePath, symbol, amount, "buy", strategy, `${mood} mood driving ${riskTolerance} buy`, mood);
                if (result.success) {
            ...
            if (positions.length > 0) {
                ...
                if (sellAmount > 0.001) {
                    const result = await executeTrade(workspacePath, symbol, sellAmount, "sell", strategy, `${mood} mood driving ${riskTolerance} sell`, mood);
                    if (result.success) {
                        state.lastTradeTime = new Date().toISOString();
                        state.totalTrades++;
                        console.log(`[economy_engine] Trade executed: ${result.message}`);
 ...
 */
export async function getEconomyState(workspacePath) {
    return loadEconomyState(workspacePath);
}
/**
 * Initialize vault with seed capital (for testing)
 */
export async function initializeVault(workspacePath, initialDeposit = 10000) {
    const vaultBridge = join(workspacePath, "skills", "soul-evolution", "tools", "vault_bridge.py");
    try {
        const { stdout } = await execFilePromise("python3", [
            vaultBridge,
...
//# sourceMappingURL=economy_engine.js.map