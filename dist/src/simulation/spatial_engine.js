 ...
 */
async function loadSpatialState(workspacePath) {
 ...
 */
async function saveSpatialState(workspacePath, state) {
    const statePath = join(workspacePath, "memory", "reality", "spatial_state.json");
 ...
 */
async function performMouseMove(workspacePath, dx, dy) {
 ...
 */
async function performKeyStroke(workspacePath, key) {
    return new Promise((resolve) => {
        const desktopBridge = join(workspacePath, "skills", "soul-evolution", "tools", "desktop_bridge.py");
 ...
 */
async function performScroll(workspacePath, direction) {
    return new Promise((resolve) => {
        const desktopBridge = join(workspacePath, "skills", "soul-evolution", "tools", "desktop_bridge.py");
 ...
 */
function generateGhostKeystroke() {
    const keys = [
 ...
 */
function isInReflexLock(physique) {
 ...
 */
export async function processSpatialInteraction(workspacePath, physique, isResearching, isExpanding, config = DEFAULT_CONFIG) {
    const now = new Date();
 ...
 */
export async function getSpatialState(workspacePath) {
    return loadSpatialState(workspacePath);
}
/**
 * Force stop spatial interaction (emergency override)
 */
export async function forceStopSpatial(workspacePath) {
 ...
 */
export function getHeadTrackingTarget(screenX, screenY) {
...
//# sourceMappingURL=spatial_engine.js.map