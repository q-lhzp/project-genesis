 ...
 */
export async function loadOSCConfig(workspacePath) {
 ...
 */
export async function saveOSCConfig(workspacePath, config) {
    const configPath = join(workspacePath, "memory", "reality", "osc_config.json");
 ...
 */
function buildOSCAddress(address) {
...
function sendOSCMessage(address, args) {
    if (!currentConfig.enabled || !udpSocket)
        return;
    try {
 ...
 */
export function sendBlendShape(name, value) {
 ...
 */
export function sendBlendShapes(blendShapes) {
    if (!currentConfig.enabled)
        return;
 ...
 */
export function sendBonePosition(name, x, y, z) {
    if (!currentConfig.enabled)
        return;
 ...
 */
export function sendBoneRotation(name, x, y, z, w) {
    if (!currentConfig.enabled)
        return;
 ...
 */
export function sendLipSync(vowel, value) {
    if (!currentConfig.enabled)
        return;
 ...
 */
export function sendLipSyncAll(vowels) {
    if (!currentConfig.enabled)
        return;
 ...
 * Close OSC bridge
 */
export function closeOSCBridge() {
    if (udpSocket) {
        udpSocket.close();
        udpSocket = null;
        isInitialized = false;
...
//# sourceMappingURL=osc_bridge.js.map