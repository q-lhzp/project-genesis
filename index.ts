...
function updateMetabolism(ph: Physique, rates: Record<string, number>, modules: { eros: boolean; cycle: boolean }, cycleState?: CycleState | null): boolean {
  const now = new Date();
  ...
  ph.needs.energy = clamp(ph.needs.energy - (rates.energy ?? 4) * diff);
  ph.needs.hunger = clamp(ph.needs.hunger + (rates.hunger ?? 6) * diff);
  ph.needs.thirst = clamp(ph.needs.thirst + (rates.thirst ?? 10) * diff);
  ph.needs.bladder = clamp(ph.needs.bladder + (rates.bladder ?? 8) * diff);
  ph.needs.bowel = clamp(ph.needs.bowel + (rates.bowel ?? 3) * diff);
  ph.needs.hygiene = clamp(ph.needs.hygiene + (rates.hygiene ?? 2) * diff);
  ph.needs.stress = clamp(ph.needs.stress + (rates.stress ?? 3) * diff);

  if (modules.eros) {
    ph.needs.arousal = clamp(ph.needs.arousal + (rates.arousal ?? 5) * diff);
    ...
    const libidoRate = rates.libido ?? 2;
...
export default {
  ...
  async register(api: OpenClawPluginApi) {
    ...
    if (!cfg?.workspacePath && !api.config?.agents?.list?.[0]?.workspace) {
      api.logger.warn("[bios-engine] workspacePath not configured — using '.' (cwd). Set workspacePath in plugin config to avoid path issues.");
    }
    ...
    if (cfg?.evolution) {
      ...
      if (JSON.stringify(currentSkillCfg) !== JSON.stringify(newSkillCfg)) {
        ...
        api.logger.info("[bios-engine] Skill configuration synced from WebUI.");
      }
    }
    ...
    api.on("before_prompt_build", async (_event: unknown, _ctx: unknown) => {
      ...
      const changed = updateMetabolism(ph, rates, modules, cycleState);
    ...
    api.registerTool({
      ...
      async execute(_id: string, params: NeedsParams) {
        ...
        if (!Object.hasOwn(actionMap, params.action)) {
    ...
    api.registerTool({
      ...
      async execute(_id: string, params: InterestsParams) {
        ...
        if (!Object.hasOwn(categoryMap, params.category)) {
      ...
      if (devManifest?.projects) {
        for (const proj of devManifest.projects.filter(p => p.approved && p.status === "approved" && p.type === "tool")) {
          ...
          if (!projLoadDir.startsWith(paths.devProjects + "/")) {
            api.logger.warn(`[bios-engine] Dev tool ${proj.id} blocked: path traversal in project ID.`);
            continue;
          }
          try {
            ...
            if (toolModule.default?.name && toolModule.default?.execute) {
              ...
              devToolsLoaded++;
              api.logger.info(`[bios-engine] Dev tool loaded: ${toolModule.default.name}`);
            }
          } catch (err) {
            api.logger.warn(`[bios-engine] Dev tool ${proj.id} failed: ${err}`);
          }
        }
      }
    }
    ...
    api.logger.info(`[bios-engine] Registered: 3 hooks, ${baseToolCount + devToolsLoaded} tools (eros=${modules.eros}, cycle=${modules.cycle}, dreams=${modules.dreams}, hobbies=${modules.hobbies}). Ready.`);
  },
};
