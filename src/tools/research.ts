...
import type { Physique, LifecycleState, SocialState, FinanceState } from "../types/simulation.js";
...
export function registerResearchTools(
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: { target: string; field?: string; value?: unknown; reset?: boolean }) {
      try {
        const physiquePath = join(workspacePath, "memory", "reality", "physique.json");

        if (params.reset) {
          if (params.target === "needs") {
            const physique = await readJson<Physique>(physiquePath);
            if (physique) {
              physique.needs = {
                energy: 50,
                hunger: 30,
        ...
        if (params.target === "location" && params.value) {
          const physique = await readJson<Physique>(physiquePath);
          if (physique) {
            physique.current_location = params.value as string;
            await writeJson(physiquePath, physique);
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: { type: string; event: string; impact?: number; duration?: number }) {
      ...
      let events: any[] = [];
      try {
        events = await readJson<any[]>(eventsFile) || [];
      } catch { }
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: { format?: string; timeframe?: string; type?: string }) {
      ...
      if (type === "all" || type === "vitality") {
        try {
          const physique = await readJson<Physique>(physiquePath);
          const lifecycle = await readJson<LifecycleState>(lifecyclePath);
      ...
      if (type === "all" || type === "social") {
        try {
          const social = await readJson<SocialState>(socialPath);
          if (format === "markdown") {
            output.push("");
            output.push("## Social Data");
            output.push("");
            output.push(`Total Contacts: ${(social as any)?.entities?.length || 0}`);
          }
        } catch { }
      }
      ...
      if (type === "all" || type === "economy") {
        try {
          const vault = await readJson<any>(vaultPath);
          if (format === "markdown") {
            output.push("");
