...
import type { Physique, LifecycleState } from "../types/simulation.js";

// Simple type definitions for vault and economy
interface VaultState {
  portfolio: { total_value_eur: number; positions: any[] };
  last_update: string;
  last_event?: string;
...
interface EconomyStateType {
  market_volatility?: string;
  last_trade_reason?: string;
}

interface AvatarConfig {
  default_vrm_path?: string;
  slot_vrm_paths?: Record<string, string>;
}

/**
 * Scenario definitions
 */
type ScenarioType = "night_recovery" | "critical_reflex" | "market_surge" | "social_event" | "hobby_session" | "full_health";
...
async function applyScenario(
...
): Promise<{ success: boolean; message: string; changes: Record<string, any> }> {
  ...
  switch (scenario) {
    case "night_recovery": {
      ...
      const ph = await readJson<Physique>(physiquePath);
      if (ph) {
        ph.needs.energy = 10;
        ...
        if (existsSync(lifecyclePath)) {
          const lc = await readJson<LifecycleState>(lifecyclePath);
          if (lc) {
            lc.is_asleep = true;
    ...
    case "market_surge": {
      ...
      const vault = await readJson<VaultState>(vaultStatePath);
      if (vault) {
      ...
      if (existsSync(economyStatePath)) {
        const eco = await readJson<EconomyStateType>(economyStatePath);
        if (eco) {
...
async function runSelfTest(
  workspacePath: string
): Promise<{ success: boolean; report: string; results: Record<string, { status: string; message: string }> }> {
  ...
  try {
    ...
    if (existsSync(avatarConfigPath)) {
      const avatar = await readJson<AvatarConfig>(avatarConfigPath);
 ...
 */
export function registerTestSuiteTools(
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: { scenario: ScenarioType }) {
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: { auto_fix?: boolean }) {
