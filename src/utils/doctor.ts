 ...
 */
function getRequiredBridges(workspacePath: string): BridgeFile[] {
  return [
 ...
 */
function getRequiredStateFiles(workspacePath: string): string[] {
  return [
 ...
 */
function getPhysiqueTemplate(): any {
  return {
 ...
 */
function getInterestsTemplate(): any {
  return {
 ...
 */
function getWardrobeTemplate(): any {
  return {
    tops: [
 ...
 */
async function runDiagnosticCheck(workspacePath: string): Promise<DiagnosticReport> {
  ...
  if (existsSync(physiquePath)) {
    ...
    try {
      const ph = await readJson<any>(physiquePath);
      if (!ph) {
        result.status = "FAIL";
  ...
  const alpacaResult: DiagnosticResult = {
  ...
  try {
    if (existsSync(modelConfigPath)) {
      const config = await readJson<any>(modelConfigPath);
  ...
  const blendResult: DiagnosticResult = {
  ...
  try {
    if (existsSync(avatarStatePath)) {
      const avatar = await readJson<any>(avatarStatePath);
  ...
  const portraitResult: DiagnosticResult = {
  ...
  const imageBridgeResult: DiagnosticResult = {
 ...
 */
export async function quickHealthCheck(workspacePath: string): Promise<boolean> {
  const report = await runDiagnosticCheck(workspacePath);
 ...
 */
export async function getDiagnosticSummary(workspacePath: string): Promise<{
  status: string;
  checks: { name: string; status: string; message: string }[];
  summary: { passed: number; failed: number; warnings: number; fixed: number };
}> {
  const report = await runDiagnosticCheck(workspacePath);
