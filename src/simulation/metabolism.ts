...
import type { Needs, Physique, LifecycleState, CycleState, LifeStage, LifeStageMultipliers } from "../types/index.js";
import { getSensation, SENSATIONS } from "../types/index.js";

export function getCyclePhase(day: number): CycleState["phase"] {
...
export function getCycleMetabolismModifiers(phase: CycleState["phase"]): Partial<Needs> {
...
export function getDefaultCycleState(): CycleState {
...
export function advanceCycleDay(cycle: CycleState): boolean {
  const now = new Date();
...
export function updateMetabolism(
  ...
  if (isNaN(diff) || diff < 0.005) return false;
