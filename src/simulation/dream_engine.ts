...
import { readJson, writeJson, appendJsonl, readJsonl } from "../utils/persistence.js";
...
interface DreamConfig {
  sleepStartHour: number;    // Default: 23 (11 PM)
  sleepEndHour: number;     // Default: 5 (5 AM)
  energyThreshold: number;   // Energy below this triggers sleep: 20
  dreamDurationMs: number;  // How long a "dream cycle" takes: 5000ms
}
...
let currentDreamState: DreamState = {
  isDreaming: false,
  sleepStartTime: null,
  dreamCount: 0,
  lastWakeTime: null,
 ...
 */
function isSleepTime(config: DreamConfig = DEFAULT_CONFIG): boolean {
  const now = new Date();
 ...
 */
async function getTodayExperiences(workspacePath: string): Promise<string[]> {
 ...
 */
function generateDreamSummary(experiences: string[]): string {
 ...
 */
function extractInsights(experiences: string[]): string[] {
 ...
 */
async function logDream(workspacePath: string, summary: string, insights: string[]): Promise<void> {
 ...
 * Write growth entry
 */
async function injectGrowth(workspacePath: string, insights: string[]): Promise<void> {
 ...
 */
export function shouldEnterDreamMode(physique: Physique, config: DreamConfig = DEFAULT_CONFIG): boolean {
  // Already dreaming
  if (currentDreamState.isDreaming) return true;
 ...
 */
export function shouldWakeUp(physique: Physique, config: DreamConfig = DEFAULT_CONFIG): boolean {
  // Currently dreaming
  if (!currentDreamState.isDreaming) return false;
 ...
 */
export async function processDreamState(
  workspacePath: string,
  physique: Physique
): Promise<{
  isDreaming: boolean;
 ...
 */
export function isSleepLocked(): boolean {
 ...
 */
export function getMorningReport(): string | null {
