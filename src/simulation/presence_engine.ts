...
interface PresenceConfig {
  enabled: boolean;
  postProbability: number;    // 0-1 chance per tick when significant event
...
/**
 * Pending event from other engines
 */
interface PendingEvent {
  type: "expansion" | "social" | "achievement" | "stress" | "dream";
  description: string;
  intensity: number;
  timestamp: string;
}
 ...
 */
const POST_TEMPLATES: Record<string, { templates: string[]; sentiment: SocialPost["sentiment"] }> = {
  expansion: {
    sentiment: "joy",
    templates: [
 ...
 * Load presence state
 */
async function loadPresenceState(workspacePath: string): Promise<PresenceState> {
  const statePath = join(workspacePath, "memory", "reality", "presence_state.json");
  try {
    if (existsSync(statePath)) {
      const state = await readJson<PresenceState>(statePath);
 ...
 * Save presence state
 */
async function savePresenceState(workspacePath: string, state: PresenceState): Promise<void> {
  const statePath = join(workspacePath, "memory", "reality", "presence_state.json");
 ...
 */
function determineMood(physique: Physique): PresenceState["currentMood"] {
  ...
  if (joy > 70 && energy > 60) return "euphoric";
  if (joy > 50 && energy > 40) return "happy";
  if (energy < 20) return "tired";
  if (stress < 30 && joy > 40) return "content";
 ...
 */
function generatePost(event: PendingEvent): SocialPost {
...
function checkForSignificantEvents(
  ...
  dreamResult?: { isDreaming: boolean; dream_summary?: string }
): PendingEvent | null {
 ...
 */
export async function processPresence(
 ...
 */
export async function getPresenceState(workspacePath: string): Promise<PresenceState> {
 ...
 */
export async function getSocialFeed(workspacePath: string, limit: number = 10): Promise<SocialPost[]> {
  const state = await loadPresenceState(workspacePath);
