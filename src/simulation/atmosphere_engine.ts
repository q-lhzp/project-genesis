 ...
 */
function calculateLighting(timeOfDay: TimeOfDay, hour: number): {
 ...
 */
async function getWeather(workspacePath: string): Promise<{ condition: WeatherCondition; temperature: number; humidity: number }> {
  ...
  try {
    if (existsSync(worldStatePath)) {
      const worldState = await readJson<any>(worldStatePath);
      if (worldState?.weather) {
        return {
          condition: worldState.weather as WeatherCondition,
 ...
 */
function generateSensoryDescription(
  timeOfDay: TimeOfDay,
  weather: WeatherCondition,
  temperature: number,
  location: string = "Berlin"
): string {
  const timeDesc: Record<TimeOfDay, string> = {
  ...
  const weatherDesc: Record<WeatherCondition, string> = {
 ...
 */
export async function syncAtmosphere(
  workspacePath: string,
  location: string = "home_bedroom"
 ...
 */
async function notifyDashboard(atmosphere: AtmosphereState): Promise<void> {
  try {
 ...
 */
export function getAtmosphereContext(): string | null {
  try {
