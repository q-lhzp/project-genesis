#!/usr/bin/env python3
"""
Soul Evolution Validator Orchestrator
Runs all validators and produces a summary report.

Usage:
  python3 soul-evolution/validators/run_all.py [--workspace-root .]

Expects standard Soul Evolution workspace layout:
  <root>/
    SOUL.md
    soul-evolution/config.json
    memory/
...
def main(workspace_root='.'):
    ...
    state_file = os.path.join(memory_dir, 'soul-state.json')
    ...
    if ws_result.get('status') == 'FAIL':
        ...
        print('Soul Evolution is NOT installed in this workspace. Pipeline must not run.')
    ...
    if os.path.exists(state_file):
        ...
        results['state'] = {
            'status': 'FAIL',
            'errors': [{'message': 'soul-state.json not found'}]
        }
    ...
    # ========================================
    print('\n' + '=' * 60)
    print('SOUL EVOLUTION VALIDATION REPORT')
    print('=' * 60)
