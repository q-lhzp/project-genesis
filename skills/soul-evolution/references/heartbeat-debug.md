...
Diagnosing why your Soul Evolution heartbeat pipeline isn't working as expected.

---

## Symptom: Main agent doesn't know what happened during heartbeat

The most common Soul Evolution issue. Your agent polls Moltbook during heartbeat,
...
Look for any jobs that mention Moltbook, feed checking, or Soul Evolution. If you
...
For Soul Evolution, you want `dmScope: "main"` (the default) so all direct
conversations share context with heartbeat runs.

---

## Symptom: Heartbeat runs but doesn't do the Soul Evolution pipeline

The heartbeat fires, but it just says HEARTBEAT_OK without running the
Soul Evolution ingestion/reflection cycle.
...
If it's empty or missing, the agent has nothing to do. Reinstall the Soul Evolution
...
## Symptom: Heartbeat runs Soul Evolution but experiences aren't persisting
...
Check `poll_interval_minutes` in `soul-evolution/config.json`. With a 3m heartbeat:
...
Also check `source_last_polled` in `memory/soul-state.json`:

```bash
cat ~/.openclaw/workspace/memory/soul-state.json | python3 -m json.tool
```
...
This is OpenClaw's native behavior, not Soul Evolution. It fires when your session
context nears the compaction threshold. The agent should write to BOTH:
1. `memory/experiences/YYYY-MM-DD.jsonl` (Soul Evolution structured)
2. `memory/YYYY-MM-DD.md` (OpenClaw native)

See `soul-evolution/SKILL.md` §10 for the full protocol.

If the agent is only writing to `.md` and skipping `.jsonl`, it means the
Soul Evolution flush redirect isn't working. Check that AGENTS.md has the
...
□ No cron jobs duplicating Soul Evolution work
□ HEARTBEAT.md exists with Soul Evolution pipeline content
□ Heartbeat prompt references HEARTBEAT.md (not overridden)
□ DM scope is "main" (default)
□ memory/ directory is writable
□ soul-state.json has valid source_last_polled timestamps
...
## Symptom: Soul Evolution is modifying the wrong agent's SOUL.md

You have multiple agents (e.g., `workspace/` and `workspace-soul-evolution-demo/`)
and Soul Evolution is installed on one of them. But you notice SOUL changes, tag
additions, or Soul Evolution artifacts appearing in the OTHER agent's workspace.

### Root cause: Cron job or heartbeat running under the wrong agent

This is the most dangerous Soul Evolution misconfiguration. It happens when:

1. A cron job (like "Soul Evolution Threshold Check") is assigned to `default` or
   the wrong agent name
2. That agent's workspace doesn't have Soul Evolution installed
3. The cron job runs Soul Evolution logic anyway, reading and writing to whatever
...
Look at the `agent` column for every Soul Evolution-related cron job. They MUST
point to the agent whose workspace has `soul-evolution/` installed.

**Dangerous pattern:**
```
Soul Evolution Heartbeat        every 5m    main      soul-evolution-demo  ← ✅ correct
Soul Evolution Threshold Check  every 15m   isolated  default       ← 🚨 WRONG
```

The second job runs under `default` (which uses `workspace/`), not the
Soul Evolution agent (which uses `workspace-soul-evolution-demo/`).
...
openclaw cron add --agent <correct-agent-name> --every 15m --prompt "Run Soul Evolution threshold check"
```
...
```bash
# In the Soul Evolution workspace — should PASS
cd workspace-soul-evolution-demo
python3 soul-evolution/validators/check_workspace.py

# In the other workspace — should FAIL (no soul-evolution/ directory)
cd ../workspace
python3 ../workspace-soul-evolution-demo/soul-evolution/validators/check_workspace.py
```
...
3. Remove any `soul-evolution/` directory or `memory/` files that were created
4. Verify the agent works normally without Soul Evolution artifacts

### Prevention

The workspace boundary check (`check_workspace.py`) runs as step 0 of every
heartbeat. It verifies `soul-evolution/SKILL.md` exists before any pipeline step.
