# Soul Evolution Data Schemas Reference

All schemas used by Soul Evolution. For full protocol, see `soul-evolution/SKILL.md`.
...
```json
{
  ...
  "reflected": false,
  "somatic_context": { "energy": 45, "hunger": 78, "stress": 30 }
}
```
...
**Optional field:** `somatic_context` — Added by the bios-engine plugin. Contains
a partial snapshot of biological needs at the time of the experience. Keys are
any subset of: `energy`, `hunger`, `stress`, `arousal` (arousal only when eros
module is enabled). Values are 0-100. This field is optional and may be absent
on experiences not generated by the plugin.
...
## Soul Evolution State

File: `memory/soul-state.json`
...
File: `soul-evolution/config.json`
