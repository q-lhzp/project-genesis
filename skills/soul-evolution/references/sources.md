# Soul Evolution Source API Reference

Soul Evolution fetches social feeds **directly** using curl. No external skills
...
### Soul Evolution Ingestion Strategy

During heartbeat:
1. Read `interests.keywords` from `soul-evolution/config.json`
...
**What counts as meaningful for Soul Evolution:**
...
Soul Evolution is designed to learn new sources. When a human says "add [platform] as
...
(`soul-evolution/references/sources.md`) following the exact same structure as
...
**Write this section directly into `soul-evolution/references/sources.md`.** This is
how you "learn" the source — future heartbeats will read this file and know
exactly how to poll it.

### Step 4: Update Config

Add the source to `soul-evolution/config.json`:
...
And add its polling state to `memory/soul-state.json`:
...
> - API reference saved to `soul-evolution/references/sources.md`
> - Config: `soul-evolution/config.json` → `sources.[source_key].enabled: true`
...
| What | File | Purpose |
|------|------|---------|
| API endpoints, auth, curl examples | `soul-evolution/references/sources.md` | Agent reads this during heartbeat to know how to call the API |
| Ingestion strategy | `soul-evolution/references/sources.md` | Agent reads this to know what to fetch and what counts as meaningful |
| Config entry | `soul-evolution/config.json` | Enables/disables source, sets env var and poll interval |
| Poll state | `memory/soul-state.json` | Tracks when source was last polled |
...
2. Read source_last_polled.<name> from soul-state.json
